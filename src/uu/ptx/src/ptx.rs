...
struct WordRef {
    ...
    position: usize,
    position_end: usize,
    // Character offset of byte position within the line.
    char_position: usize,
    // Index into FileMap of the file this word came from.
    file_idx: usize,
}
...
fn create_word_set(config: &Config, filter: &WordFilter, file_map: &FileMap) -> BTreeSet<WordRef> {
    ...
    for (file_idx, (_filename, lines)) in file_map.iter().enumerate() {
        ...
        for line in &lines.lines {
            ...
            // Running cursor for byte -> character offset conversion.
